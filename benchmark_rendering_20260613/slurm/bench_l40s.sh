...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/benchmark_rendering_20260613/logs/%j_bench_l40s.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/benchmark_rendering_20260613/logs/%j_bench_l40s.err
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=cross \
  ...
  distill.teacher_data_dir=benchmark-tmp-l40s \
  distill.teacher_logdir=runs distill.teacher_resume=wheel-wrench-teacher-s1 \
  train.params.config.user_prefix=bench-l40s wandb_activate=False experiment=bench"
