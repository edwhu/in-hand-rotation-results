...
#SBATCH --time=6:00:00
#SBATCH --gres=gpu:1
#SBATCH --nodelist=ee-a6000-1.grasp.maas
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/benchmark_rendering_20260613/logs/%j_bench_a6000.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/benchmark_rendering_20260613/logs/%j_bench_a6000.err
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=cross \
  ...
  distill.teacher_data_dir=benchmark-tmp-a6000 \
  distill.teacher_logdir=runs distill.teacher_resume=wheel-wrench-teacher-s1 \
  train.params.config.user_prefix=bench-a6000 wandb_activate=False experiment=bench"
