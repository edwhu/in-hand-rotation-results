#!/bin/bash
#SBATCH --job-name=bc-baoding-prop-only
...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_bc_prop_only.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_bc_prop_only.err
...
echo "[$(date)] BC baoding prop-only starting"
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=ball \
  ...
  train.params.config.user_prefix=bc-baoding-prop-only \
  experiment=bc-baoding-prop-only wandb_activate=False"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON}

echo "[$(date)] BC baoding prop-only done."
