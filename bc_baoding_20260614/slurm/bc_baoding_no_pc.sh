#!/bin/bash
#SBATCH --job-name=bc-baoding-no-pc
...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_bc_no_pc.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_bc_no_pc.err
...
echo "[$(date)] BC baoding no-pc starting"
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=ball \
  ...
  train.params.config.user_prefix=bc-baoding-no-pc \
  experiment=bc-baoding-no-pc wandb_activate=False"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON}

echo "[$(date)] BC baoding no-pc done."
