#!/bin/bash
#SBATCH --job-name=collect-baoding-w0
...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_collect_w0.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_collect_w0.err
...
echo "[$(date)] Worker 0 starting"

COMMON="headless=True task=AllegroArmMOAR task.env.objSet=ball \
  task.env.legacy_obs=False task.env.observationType=full_stack_baoding \
  ...
  task.env.numEnvs=64 \
  train.params.config.user_prefix=bc-baoding-collect \
  ...
  distill.learn.max_iterations=500"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON} distill.worker_id=0

echo "[$(date)] Worker 0 done."
