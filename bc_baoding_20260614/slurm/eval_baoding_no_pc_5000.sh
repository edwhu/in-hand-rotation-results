...
#SBATCH --job-name=eval-baoding-no-pc-5000
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_eval-baoding-no-pc-5000.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_eval-baoding-no-pc-5000.err
...
echo "[$(date)] eval-baoding-no-pc-5000 starting"
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=ball \
  ...
  distill.student_resume=runs/student/bc-baoding-no-pc/model_bc_5000.pth \
  train.params.config.minibatch_size=1024 \
  train.params.config.central_value_config.minibatch_size=1024 \
  train.params.config.user_prefix=eval-baoding-no-pc-5000 \
  experiment=eval-baoding-no-pc-5000 wandb_activate=False"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON}

echo "[$(date)] eval-baoding-no-pc-5000 done."
