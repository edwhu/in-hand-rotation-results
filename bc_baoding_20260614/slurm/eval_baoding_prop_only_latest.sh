#!/bin/bash
#SBATCH --job-name=eval-baoding-prop-only-latest
...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_eval_baoding_prop_only_latest.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_baoding_20260614/logs/%j_eval_baoding_prop_only_latest.err
...
echo "[$(date)] Eval baoding prop-only latest starting"
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=ball \
  ...
  distill.student_resume=runs/student/bc-baoding-prop-only/model_bc_14250.pth \
  train.params.config.minibatch_size=1024 \
  train.params.config.central_value_config.minibatch_size=1024 \
  train.params.config.user_prefix=eval-baoding-prop-only-latest \
  experiment=eval-baoding-prop-only-latest wandb_activate=False"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON}

echo "[$(date)] Eval baoding prop-only latest done."
