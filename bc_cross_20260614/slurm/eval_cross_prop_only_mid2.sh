#!/bin/bash
#SBATCH --job-name=eval-cross-prop-only-mid2
...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_cross_20260614/logs/%j_eval_prop_only_mid2.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_cross_20260614/logs/%j_eval_prop_only_mid2.err
...
echo "[$(date)] Eval cross prop-only mid2 starting"
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=cross \
  ...
  train.params.config.user_prefix=eval-cross-prop-only-mid2 \
  experiment=eval-cross-prop-only-mid2 wandb_activate=False"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON}

echo "[$(date)] Eval cross prop-only mid2 done."
