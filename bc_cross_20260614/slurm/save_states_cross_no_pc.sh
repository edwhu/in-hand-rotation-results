#!/bin/bash
#SBATCH --job-name=save-states-cross-no-pc
...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_cross_20260614/logs/%j_save_states_no_pc.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_cross_20260614/logs/%j_save_states_no_pc.err
...
echo "[$(date)] Save states no-pc starting"
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=cross \
  ...
  train.params.config.user_prefix=save-states-cross-no-pc \
  experiment=save-states-cross-no-pc wandb_activate=False"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON}

echo "[$(date)] Save states no-pc done. States at ${STATES_PATH}"
