#!/bin/bash
#SBATCH --job-name=save-states-cross-prop
...
#SBATCH --output=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_cross_20260614/logs/%j_save_states_prop_only.out
#SBATCH --error=/mnt/kostas-graid/sw/envs/edward/in-hand-rotation-results/bc_cross_20260614/logs/%j_save_states_prop_only.err
...
echo "[$(date)] Save states prop-only starting"
...
COMMON="headless=True task=AllegroArmMOAR task.env.objSet=cross \
  ...
  distill.save_states=True distill.states_path=${STATES_PATH} \
  ...
  train.params.config.user_prefix=save-states-cross-prop-only \
  experiment=save-states-cross-prop-only wandb_activate=False"

CUDA_VISIBLE_DEVICES=0 python -u ./isaacgymenvs/train_distillation.py ${COMMON}

echo "[$(date)] Save states prop-only done. States at ${STATES_PATH}"
